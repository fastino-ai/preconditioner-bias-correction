#!/usr/bin/env bash
# Compute-matched AdamW BC at gradient batch size 512:
#   BC @ A=512, B=512 with rolling-window B, fixed alpha=0.5
#
# Reuses runs/adamw_cm_std512/std_history.json as the baseline.
...
RUN_DIR="runs/adamw_cm_bc_rolling_b512_alpha05_fixed"
LOG="$RUN_DIR/log.txt"
if [[ -e "$RUN_DIR" ]]; then
...
echo "=== BC fixed alpha=$ALPHA rolling-B, A=512 B=512, data_seed=$DATA_SEED ===  $(date -u +%FT%TZ)" | tee "$LOG"
echo "cf mode (NO inverse correction), fixed alpha, no clip, stream_grads" | tee -a "$LOG"
...
python3 -u -m bcopt.plotting.compare --run_dir "$RUN_DIR" --optimizer "AdamW (compute-matched BC batch=512 fixed alpha=0.5)" 2>&1 | tee -a "$LOG"
[[ "$ec" -eq 0 ]] || exit 1
