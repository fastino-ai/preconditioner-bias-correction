...
RUN_NAME="${RUN_NAME:-adamw_cm_bc_rolling_alpha1}"
...
python3 -u -m bcopt.plotting.compare --run_dir "$RUN_DIR" --optimizer "AdamW (compute-matched BC α_max=1.0)" 2>&1 | tee -a "$LOG"
[[ "$ec" -eq 0 ]] || exit 1
