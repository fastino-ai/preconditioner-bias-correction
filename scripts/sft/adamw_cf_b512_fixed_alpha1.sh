...
#   BC @ A=512, B=512 with rolling-window B, fixed alpha=1.0
#
...
RUN_DIR="runs/adamw_cm_bc_rolling_b512_alpha1_fixed"
LOG="$RUN_DIR/log.txt"
if [[ -e "$RUN_DIR" ]]; then
...
python3 -u -m bcopt.plotting.compare --run_dir "$RUN_DIR" --optimizer "AdamW (compute-matched BC batch=512 fixed alpha=1.0)" 2>&1 | tee -a "$LOG"
[[ "$ec" -eq 0 ]] || exit 1
