...
python3 -u -m bcopt.plotting.compare --run_dir "$FULL_DIR" --optimizer "Shampoo CM b512 FULL BC lr=2e-5 vs std lr=2e-5" 2>&1 | tee -a "$FULL_LOG"

echo "=== Running 5000-example eval for saved Shampoo checkpoints ===" | tee -a "$FULL_LOG"
python3 -u -m bcopt.eval.shampoo_5k 2>&1 | tee -a "$FULL_LOG"
