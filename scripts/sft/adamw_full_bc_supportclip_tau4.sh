...
RUN_NAME="${RUN_NAME:-adamw_full_supportclip4}"
...
python3 -u -m bcopt.plotting.compare --run_dir "$RUN_DIR" --optimizer "AdamW (full BC w/ support clip τ=$TAU)" 2>&1 | tee -a "$LOG"
[[ "$ec" -eq 0 ]] || exit 1
