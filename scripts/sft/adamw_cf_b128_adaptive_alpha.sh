...
RUN_NAME="${RUN_NAME:-adamw_cf_adaptive}"
...
echo "=== adamw cf with ADAPTIVE alpha (cos(s_A,s_B) gating) ==="    | tee "$LOG"
...
python3 -u -m bcopt.plotting.compare --run_dir "$RUN_DIR" --optimizer "AdamW (cf adaptive α, no clip)" 2>&1 | tee -a "$LOG"
[[ "$ec" -eq 0 ]] || exit 1
