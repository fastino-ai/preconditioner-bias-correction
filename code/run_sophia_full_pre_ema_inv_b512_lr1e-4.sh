#!/usr/bin/env bash
# Sophia FULL BC with PRE-EMA delta-method inverse variance correction.
# Compute-matched recipe identical to
#   ../runs/sophia_cm_bc_full_b512_lr2e-5_m8_detached_v2
# except the learning rate is 1e-4 (vs 2e-5) and the inverse-correction
# block uses the pre-EMA delta-method on bar_r_B = mean_j r_{B_j}
# (where r_{B_j} = g_GNB,B_j**2) instead of the post-EMA correction on
# bar_p_t.
#
# Setup: A=512 (training gradient / momentum), B=512 rolling-window
# (Hessian via GNB), denom_bs=512 so Sophia's rho*bs*h denominator
# matches the gradient batch.
set -u
cd "$(dirname "$0")"
...
RUN_DIR="../runs/sophia_full_pre_ema_inv_b512_lr1e-4"
LOG="$RUN_DIR/log.txt"
...
python3 -u train_sophia_pre_ema_inv.py \
  --mode full \
...
python3 -u plot_results.py --run_dir "$RUN_DIR" \
  --optimizer "Sophia-G FULL BC pre-EMA inv-corr lr=1e-4 vs std lr=2e-5" 2>&1 | tee -a "$LOG"
[[ "$ec" -eq 0 ]] || exit 1
