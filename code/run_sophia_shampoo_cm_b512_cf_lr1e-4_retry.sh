...
run_sophia() {
  ...
  python3 -u plot_results.py --run_dir "$run_dir" --optimizer "Sophia-G CM b512 BC-CF lr=1e-4 vs std lr=2e-5" 2>&1 | tee -a "$log"
  [[ "$ec2" -eq 0 ]] || exit 1
}
...
run_shampoo() {
  ...
  [[ -f "$run_dir/cf_history.json" ]] && mv "$run_dir/cf_history.json" "$run_dir/full_history.json"
  python3 -u plot_results.py --run_dir "$run_dir" --optimizer "Shampoo CM b512 BC-CF lr=1e-4 vs std lr=2e-5" 2>&1 | tee -a "$log"
  [[ "$ec2" -eq 0 ]] || exit 1
}
