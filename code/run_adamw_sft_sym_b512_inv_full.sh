#!/usr/bin/env bash
# AdamW sym-hybrid SFT @ b=512 lr=2e-5: ONLY inv and full modes.
# (std and cf intentionally skipped per user request.)
set -u
cd "$(dirname "$0")"
...
DATA_SEED=99
MICRO=8
NUM_MICRO=32   # 2 * 32 * 8 = 512 examples/step
...
ec_total=0
for MODE in inv full; do
  run_mode "$MODE" || ec_total=$?
done

echo ""
echo "=== AdamW SFT sym-hybrid b=512 lr=2e-5 (inv, full) summary ==="
for MODE in inv full; do
  python3 -c "
import json, os
