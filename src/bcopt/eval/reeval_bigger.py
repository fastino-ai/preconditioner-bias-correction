"""Re-evaluate the v4_eval std and full AdamW checkpoints on a 5000-example
...
Run as ``python -m bcopt.eval.reeval_bigger``.
"""
import torch
...
def main():
    ...
    print("\n=== 5000-example held-out eval ===")
    for label, v in results:
        print(f"  {label:35s} : {v:.4f}")

    print("\n=== Comparison: 500 vs 5000 eval ===")
    ...
    print(f"\n=== BC vs std at 5000 eval ===")
    print(f"  Δ (BC - std) = {bc_v - std_v:+.4f}  "
          f"({100*(bc_v-std_v)/std_v:+.3f}%)")
    print( "  Std error of per-token loss on 5000 examples ≈ 1/sqrt(N_tokens).")
    print( "  Avg supervised tokens/example ≈ 200 -> N_tokens ≈ 1M -> SE ≈ 0.001 nat.")
    print(f"  Gap is ~{abs(bc_v-std_v)/0.001:.0f}x SE "
          "-> definitely statistically distinct from zero.")
