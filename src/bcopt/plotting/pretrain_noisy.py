"""Headline AdamW BC plot for mixed-quality FineWeb-Edu pretraining.

Reproduces the right panel of Figure 1 in the paper: 20% of the training
sequences are span-replaced; the held-out eval set is clean. Shows training
loss curves and final clean-eval bar for std AdamW vs BC AdamW (LOO+Jensen).

Run as ``python -m bcopt.plotting.pretrain_noisy`` from the repo root, or
...
def main():
    ...
    ap.add_argument("--std_dir",
                    default=str(repo_root / "runs"
                                / "adamw_pretrain_std_b512_lr6e-4_noisyq0.2"))
    ap.add_argument("--bc_dir",
                    default=str(repo_root / "runs"
                                / "adamw_pretrain_loo_hybrid_sqm_jensen_b512_emb6e-4_dense9e-4_floor0.2_noisyq0.2"))
    ap.add_argument("--out",
                    default=str(repo_root / "runs"
                                / "bc_vs_std_noisy_q0.2.png"))
